...
let root = null;
let decoded_string = "";
function buildHoffTreeAndGenerateHoofCode(frqMapArr) {
...
function generateFrequencyMapArr(s) {
    ...
    for (const uset of setS) {
        ...
        let frq = s.match(re);
        let mapNode = {
            freq: (frq === null || frq === void 0 ? void 0 : frq.length) ? frq.length : 0,
            char: uset,
            left: null,
            right: null,
        };
...
function decodeHoofCode(_root, code) {
    if (_root.left == null &&
        _root.right == null &&
